...
class Bot(discord.Client):
    ...
    async def on_message(self, message):
        ...
        if self.content.startswith(f'{p}cleanup'):
            ...
            if not self.permissions.manage_messages: return await self.channel.send(embed=discord.Embed(color=0xff0000).set_footer(text='У вас недостаточно прав.', icon_url=icons['error']))
...
'''
        ...
        if self.content.startswith(f'{p}ban'):
            ...
            if not self.permissions.ban_members: return await self.channel.send(embed=discord.Embed(color=0xff0000).set_footer(text='У вас недостаточно прав.', icon_url=icons['error']))
        ...
        if self.content.startswith(f'{p}myname'):
            ...
            #if not self.permissions.manage_nicknames: return await self.channel.send(embed=discord.Embed(color=0xff0000).set_footer(text='У вас недостаточно прав.', icon_url=icons['error']))
        ...
        if self.content.startswith(f'{p}status'):
            ...
            if not self.permissions.manage_guild: return await self.channel.send(embed=discord.Embed(color=0xff0000).set_footer(text='У вас недостаточно прав.', icon_url=icons['error']))
        ...
        if self.content.startswith(f'{p}config'):
            ...
            if not arg[1] == 'help':
                if arg[1] == 'prefix':
                    ...
                    if Data.config.upload(self.guild): return await _msg.add_reaction(react['suc'])
                    else: return await _msg.add_reaction(react['err'])
                if arg[1] == 'max-warns':
                ...
                if arg[1] == 'admin':
                    ...
                    try: arg[2]
                    except: return await channel.send(embed=discord.Embed(color=0xff0000, description=f'{p}config admin add [@пользователь]\n{p}config admin remove [@пользователь]'))

                    if arg[2] == 'add':

                        _msg = await self.wait_for('message', check=check)

                        _username = Data.user.load(_msg, self.guild)

                        data['admins'].append(_username)
